    ...
    return (
        <div className='pb-10 md:py-20 mx-2'>
            ...
            <div className="relative w-full max-w-3xl overflow-hidden m-auto rounded-xl">
                {/* BG Image */}
                <img className='' src={assets.image_w_bg} style={{ clipPath: `inset(0 ${100.2 - sliderPosition}% 0 0)` }} alt="" />
                {/* Forgroung Image */}
                <img className='absolute top-0 left-0 w-full h-full' src={assets.image_wo_bg} style={{ clipPath: `inset(0 0 0 ${sliderPosition}% )` }} alt="" />

                {/* Slider */}
                <input className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full z-10 slider' type="range" name="" id="" min={0} max={100} value={sliderPosition} onChange={handleSlider} />
            </div>
        </div>
    )
}
